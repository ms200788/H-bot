...
# Minimal inline comments; production-ready.
...
# -------------------------
db: sqlite3.Connection  # global annotation
...
# -------------------------
# Delete job executor & restoration
# -------------------------
...
# -------------------------
# Owner-only wrapper (simple)
# -------------------------
def is_owner(user_id:int)->bool:
    return user_id == OWNER_ID

# -------------------------
# Utility to build channel buttons
# -------------------------
...
async def _on_choose_protect(call: types.CallbackQuery, callback_data: dict):
    ...
    try:
        ...
        if OWNER_ID in active_uploads:
            ...
            await call.message.answer("Upload session expired or missing.")
    except Exception:
...
async def cmd_setmessage(message: types.Message):
    ...
    if message.reply_to_message and (not args_raw):
        # expecting: /setmessage start or /setmessage help in reply to message
        # parse 'start' or 'help' from command itself if present
        ...
        if message.reply_to_message.text:
            ...
            return
    # inline usage: /setmessage start Hello {username}
    parts = args_raw.strip().split(" ", 1)
    if not parts or not parts[0]:
        await message.reply("Usage: /setmessage start <text> OR reply to a text with `/setmessage start`.")
        return
    target = parts[0].lower()
    if len(parts) == 1:
        await message.reply("Provide the message text after target or reply to a message.")
        return
    txt = parts[1]
...
async def cmd_setimage(message: types.Message):
    if not is_owner(message.from_user.id):
        await message.reply("Unauthorized.")
        return
    # Accept reply to photo or document or sticker
    if not message.reply_to_message:
        await message.reply("Reply to a photo/document with `/setimage start` or `/setimage help`.")
        return
    parts = message.get_args().strip().split()
    target = parts[0].lower() if parts else "start"
    # image may be in photo, document (image), or sticker
    rt = message.reply_to_message
    file_id = None
    if rt.photo:
        file_id = rt.photo[-1].file_id
    elif rt.document:
        # accept if document is an image
...
async def cmd_setchannel(message: types.Message):
    ...
    args = message.get_args().strip()
    if not args:
        await message.reply("Usage: /setchannel <name> <channel_link> OR /setchannel none (to clear all)")
        return
    if args.lower() == "none":
    ...
    parts = args.split(" ", 1)
    if len(parts) < 2:
        await message.reply("Provide both name and link: /setchannel Updates https://t.me/MyUpdates")
        return
    ...
    try:
        ...
        arr = []
    # allow update if same name or link exists
    updated = False
    for entry in arr:
...
async def cmd_setforcechannel(message: types.Message):
    ...
    args = message.get_args().strip()
    if not args:
        await message.reply("Usage: /setforcechannel <name> <channel_link> OR /setforcechannel none (to clear all)")
        return
    if args.lower() == "none":
    ...
    parts = args.split(" ", 1)
    if len(parts) < 2:
        await message.reply("Provide both name and link.")
        return
    name, link = parts[0].strip(), parts[1].strip()
    try:
...
async def cb_help(call: types.CallbackQuery):
    try:
        ...
        # if image exists, show as photo with caption
        ...
        logger.exception("Failed to handle help callback")
        try:
            await call.message.answer("Failed to show help.")
        except Exception:
            pass
